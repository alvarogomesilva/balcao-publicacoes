...
import { useRegisterSentinel } from "../hooks/register-sentinel-service";
...
export function ModalRegisterSentinel({ isOpen, setIsOpen }: ModalProps) {
  ...
  async function handleRegisterSentinel(data: RegisterPublication) {
    try {
      await registerSentinel(data);
      reset();
      setIsOpen(false);
    } catch (error) {
      toast.error(getErrorMessage(error, "Não foi possível cadastrar o sentinela."));
    }
  }
      ...
      <Portal>
        ...
        <Dialog.Positioner>
          <Dialog.Content>
            ...
            <Dialog.Body pb="8" as="form" onSubmit={handleSubmit(handleRegisterSentinel)}>
              <Controller
                control={control}
                name="active"
                render={({ field }) => (
