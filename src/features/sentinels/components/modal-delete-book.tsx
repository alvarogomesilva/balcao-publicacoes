...
import { useDeleteSentinel } from "../hooks/use-delete-book";
...
export function ModalDeleteBook({
  ...
  const { deleteSentinel } = useDeleteSentinel();

  async function handleDeletePublication(id: string) {
    try {
      await deleteSentinel(id);
      closeButton.current?.click();
    } catch (error) {
      toast.error(getErrorMessage(error, "Não foi possível excluir o sentinela."));
    }
  }
